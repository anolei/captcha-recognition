...
def create_captcha_image(chars, width=200, height=70):
    ...
    # image.show()
    image.save(os.path.join('F:\\毕业设计\\solving_captchas_code_examples\\testset', ''.join(chars)+'.png'))  # 保存验证码图片
    return image
...
def _draw_character(draw, c):
    ...
    font = ImageFont.truetype('C:\\Windows\\Fonts\\Times New Roman\\msyh.ttc', 45)
...
def wrap(img, w, h):
    """图片扭曲"""
    dx = w * random.uniform(0.1, 0.1)
    dy = h * random.uniform(0.1, 0.1)
